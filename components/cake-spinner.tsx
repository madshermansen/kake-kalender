...
const cakeTypes = [
  { name: 'Sjokoladefondant', emoji: '🍫', note: 'Rich, gooey, and always a crowd-pleaser.' },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        <header className="mx-auto mb-8 max-w-2xl text-center">
          ...
          <p className="mt-3 text-muted-foreground">La kakespinneren velge din neste søte fristelse.</p>
        </header>

        <section className="mx-auto max-w-4xl rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-8" aria-live="polite">
          <div className="mb-5 flex items-center justify-between"><span className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Kakekasse #001</span><span className="rounded-full bg-accent px-3 py-1 text-xs font-bold text-primary">Sjeldent dessertslipp</span></div>
          ...
          <div className="mt-7 text-center"><p className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">{isSpinning ? 'Åpner kasse ...' : 'Kasse åpnet'}</p><h2 className="mt-2 font-serif text-3xl font-bold sm:text-4xl">{selected.name}</h2><p className="mx-auto mt-2 max-w-md text-muted-foreground">{selected.note}</p><button type="button" onClick={spin} disabled={isSpinning} className="mt-6 inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 font-bold text-primary-foreground shadow-sm transition hover:opacity-90 disabled:cursor-wait disabled:opacity-70"><Dices data-icon="inline-start" /> {isSpinning ? 'Pakker ut ...' : 'Åpne kakekasse'}</button></div>
        </section>

        <section className="mx-auto mt-8 grid max-w-2xl gap-3 sm:grid-cols-2" aria-label="Kakeidéer">
        ...
        <div className="mt-8 flex justify-center gap-2 text-sm text-muted-foreground"><CakeSlice className="text-primary" /> Ingen feil svar, bare kake.</div>
      </div>
    </main>
  )
}
