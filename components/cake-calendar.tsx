    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        <header className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <nav className="order-3 flex w-full items-center gap-1 rounded-xl border border-border bg-muted p-1 sm:order-none sm:w-auto" aria-label="Cake calendar pages"><Link href="/" className={`flex-1 rounded-lg px-4 py-2.5 text-center text-sm font-bold transition sm:flex-none ${page === 'calendar' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>Calendar</Link><Link href="/leaderboard" className={`flex-1 rounded-lg px-4 py-2.5 text-center text-sm font-bold transition sm:flex-none ${page === 'leaderboard' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>Leaderboard</Link><Link href="/suggestions" className={`flex-1 rounded-lg px-4 py-2.5 text-center text-sm font-bold transition sm:flex-none ${page === 'suggestions' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>Suggestions</Link></nav><div className="flex flex-wrap items-center gap-2">{isAdmin ? <><span className="rounded-full bg-accent px-4 py-2 text-sm font-semibold">Admin mode</span><button onClick={async () => { await signOut(); router.refresh() }} className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-bold"><LogOut size={16} /> Sign out</button><button onClick={() => setIsAdding(true)} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground shadow-sm transition hover:opacity-90"><Plus size={18} /> Add cake day</button></> : <><Link href="/suggestions" className="rounded-full border border-primary px-3 py-2 text-xs font-bold text-primary transition hover:bg-accent">Suggest a cake</Link><a href="/sign-in" className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-bold"><LogIn size={16} /> Admin sign in</a></>}</div>
        </header>
