      ...
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
          <section className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-6">
            ...
            <div className="grid grid-cols-7 gap-1 text-center">{weekDays.map((day) => <div key={day} className="pb-3 font-mono text-xs font-bold uppercase text-muted-foreground">{day}</div>)}{days.map((day, index) => { const key = day ? localDateKey(day) : `empty-${index}`; const dayEvents = day ? events.filter((event) => event.cake_date === key) : []; const isWeekend = day ? day.getDay() === 5 || day.getDay() === 6 : false; return <button key={key} disabled={!day} onClick={() => day && setSelectedDate(key)} className={`min-h-20 rounded-xl p-2 text-left transition ${day && selectedDate === key ? 'bg-primary text-primary-foreground' : isWeekend ? 'bg-muted/70 text-muted-foreground hover:bg-muted' : 'hover:bg-muted'} ${day && key === localDateKey(today) ? 'ring-2 ring-primary ring-offset-2 ring-offset-card' : ''} ${!day ? 'cursor-default' : ''}`}><span className="text-sm font-semibold">{day?.getDate()}</span>{dayEvents.map((event) => <span key={event.id} className={`mt-2 block truncate text-xs ${selectedDate === key ? 'text-primary-foreground/80' : isWeekend ? 'text-muted-foreground' : 'text-primary'}`} title={event.person_name}><span aria-hidden="true">🍰 </span>{event.person_name}</span>)}</button> })}</div>
          </section>
