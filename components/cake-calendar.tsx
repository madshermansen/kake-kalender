...
export function CakeCalendar({ initialEvents, initialSuggestions, isAdmin, page = 'calendar' }: { initialEvents: CakeEvent[]; initialSuggestions: CakeSuggestion[]; isAdmin: boolean; page?: 'calendar' | 'leaderboard' | 'suggestions' }) {
  ...
  const [activeTab, setActiveTab] = useState<'leaderboard' | 'suggestions'>(page === 'suggestions' ? 'suggestions' : 'leaderboard')
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        <header className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <nav className="order-3 flex w-full items-center gap-1 rounded-xl border border-border bg-muted p-1 sm:order-none sm:w-auto" aria-label="Cake calendar pages"><Link href="/" className={`rounded-lg px-3 py-2 text-xs font-bold ${page === 'calendar' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>Calendar</Link><Link href="/leaderboard" className={`rounded-lg px-3 py-2 text-xs font-bold ${page === 'leaderboard' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>Leaderboard</Link><Link href="/suggestions" className={`rounded-lg px-3 py-2 text-xs font-bold ${page === 'suggestions' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>Suggestions</Link></nav><div className="flex flex-wrap items-center gap-2">{isAdmin ? <><span className="rounded-full bg-accent px-4 py-2 text-sm font-semibold">Admin mode</span><button onClick={async () => { await signOut(); router.refresh() }} className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-bold"><LogOut size={16} /> Sign out</button><button onClick={() => setIsAdding(true)} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground shadow-sm transition hover:opacity-90"><Plus size={18} /> Add cake day</button></> : <><button onClick={() => { setActiveTab('suggestions'); setIsSuggesting((value) => !value) }} className="rounded-full border border-primary px-3 py-2 text-xs font-bold text-primary transition hover:bg-accent">{isSuggesting ? 'Close suggestion' : 'Suggest a cake'}</button><a href="/sign-in" className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-bold"><LogIn size={16} /> Admin sign in</a></>}</div>
        </header>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)] lg:items-start">
          ...
          <aside className="order-2 flex flex-col gap-6">
            <div className="flex rounded-xl border border-border bg-muted p-1" role="tablist" aria-label="Cake details">
              <button role="tab" aria-selected={activeTab === 'leaderboard'} onClick={() => setActiveTab('leaderboard')} className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold transition ${activeTab === 'leaderboard' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>Leaderboard</button>
              <button role="tab" aria-selected={activeTab === 'suggestions'} onClick={() => setActiveTab('suggestions')} className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold transition ${activeTab === 'suggestions' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>Suggestions</button>
            </div>
        ...
        {(isSuggesting || activeTab === 'suggestions') && <section className={`${activeTab === 'suggestions' ? 'block' : 'hidden'} order-2 rounded-2xl border border-border bg-card p-5 shadow-sm`}><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-xs font-bold uppercase tracking-wider text-primary">Have a sweet idea?</p><h2 className="mt-1 font-serif text-2xl font-bold">Suggest a cake day</h2><p className="mt-1 text-sm text-muted-foreground">Tell the admin who should bring cake and why.</p></div><button onClick={() => setIsSuggesting(false)} className="rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground">Close form</button></div><form onSubmit={submitSuggestion} className="mt-5 grid gap-4 rounded-xl bg-muted p-4 sm:grid-cols-2"><label className="flex flex-col gap-2 text-sm font-semibold">Date<input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} required className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><label className="flex flex-col gap-2 text-sm font-semibold">Your name<input value={suggestionName} onChange={(event) => setSuggestionName(event.target.value)} required className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><label className="flex flex-col gap-2 text-sm font-semibold sm:col-span-2">Reason<textarea value={suggestionReason} onChange={(event) => setSuggestionReason(event.target.value)} placeholder="Birthday, team celebration, or just because..." required rows={3} className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><button className="rounded-full bg-foreground px-5 py-3 text-sm font-bold text-background sm:col-span-2">Send suggestion</button></form></section>}
