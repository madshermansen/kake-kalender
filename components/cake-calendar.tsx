...
import { CakeSlice, ChevronLeft, ChevronRight, Plus, Trash2 } from 'lucide-react'
...
export function CakeCalendar({ initialEvents }: { initialEvents: CakeEvent[] }) {
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        <header className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button onClick={() => setIsAdding(true)} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground shadow-sm transition hover:opacity-90"><Plus size={18} /> Add cake day</button>
        </header>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
          <section className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-6">
            ...
            <div className="grid grid-cols-7 gap-1 text-center">{weekDays.map((day) => <div key={day} className="pb-3 font-mono text-xs font-bold uppercase text-muted-foreground">{day}</div>)}{days.map((day, index) => { const key = day ? localDateKey(day) : `empty-${index}`; const dayEvents = day ? events.filter((event) => event.cake_date === key) : []; return <button key={key} disabled={!day} onClick={() => day && setSelectedDate(key)} className={`min-h-20 rounded-xl p-2 text-left transition ${day && selectedDate === key ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'} ${day && key === localDateKey(today) ? 'ring-2 ring-primary ring-offset-2 ring-offset-card' : ''} ${!day ? 'cursor-default' : ''}`}><span className="text-sm font-semibold">{day?.getDate()}</span>{dayEvents.map((event) => <span key={event.id} className={`mt-2 block truncate text-xs ${selectedDate === key ? 'text-primary-foreground/80' : 'text-primary'}`} title={event.person_name}><span aria-hidden="true">🍰 </span>{event.person_name}</span>)}</button> })}</div>
          </section>

          <aside className="rounded-2xl border border-border bg-card p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><div><p className="font-mono text-xs font-bold uppercase tracking-wider text-muted-foreground">Coming up</p><h2 className="mt-1 font-serif text-2xl font-bold">Next slices</h2></div><CakeSlice className="text-primary" /></div>{upcoming.length ? <div className="flex flex-col gap-4">{upcoming.slice(0, 6).map((event) => <div key={event.id} className="group flex items-start gap-3"><div className="min-w-12 rounded-lg bg-accent px-2 py-2 text-center"><span className="block text-xs font-bold uppercase text-muted-foreground">{new Date(`${event.cake_date}T12:00:00`).toLocaleDateString(undefined, { month: 'short' })}</span><span className="block text-xl font-bold">{new Date(`${event.cake_date}T12:00:00`).getDate()}</span></div><div className="min-w-0 flex-1"><p className="font-semibold">{event.person_name}</p><p className="truncate text-sm text-muted-foreground">{event.notes || 'Cake day!'}</p></div><button aria-label={`Remove ${event.person_name}`} onClick={() => removeCake(event.id)} className="invisible rounded p-1 text-muted-foreground hover:text-destructive group-hover:visible"><Trash2 size={16} /></button></div>)}</div> : <p className="rounded-xl bg-muted p-4 text-sm text-muted-foreground">No upcoming cake days yet. Add the first one!</p>}</aside>
        </div>
