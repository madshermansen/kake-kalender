    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        <div className="flex flex-col gap-8">
          <section className="order-3 rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5 lg:order-3 lg:max-w-[680px] lg:self-center">
        ...
        <section className="order-2 rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-xs font-bold uppercase tracking-wider text-primary">Have a sweet idea?</p><h2 className="mt-1 font-serif text-2xl font-bold">Suggest a cake day</h2><p className="mt-1 text-sm text-muted-foreground">Tell the admin who should bring cake and why.</p></div><button onClick={() => setIsSuggesting((value) => !value)} className="rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground">{isSuggesting ? 'Close form' : 'Make a suggestion'}</button></div>{isSuggesting && <form onSubmit={submitSuggestion} className="mt-5 grid gap-4 rounded-xl bg-muted p-4 sm:grid-cols-2"><label className="flex flex-col gap-2 text-sm font-semibold">Date<input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} required className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><label className="flex flex-col gap-2 text-sm font-semibold">Your name<input value={suggestionName} onChange={(event) => setSuggestionName(event.target.value)} required className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><label className="flex flex-col gap-2 text-sm font-semibold sm:col-span-2">Reason<textarea value={suggestionReason} onChange={(event) => setSuggestionReason(event.target.value)} placeholder="Birthday, team celebration, or just because..." required rows={3} className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><button className="rounded-full bg-foreground px-5 py-3 text-sm font-bold text-background sm:col-span-2">Send suggestion</button></form>}</section>
