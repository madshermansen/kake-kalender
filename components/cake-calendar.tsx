  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        {page === 'suggestions' && <section className="order-2 rounded-2xl border border-border bg-card p-5 shadow-sm lg:col-span-2"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-xs font-bold uppercase tracking-wider text-primary">Have a sweet idea?</p><h2 className="mt-1 font-serif text-2xl font-bold">Suggest a cake day</h2><p className="mt-1 text-sm text-muted-foreground">Tell the admin who should bring cake and why.</p></div><button type="button" onClick={() => setIsSuggesting((current) => !current)} className="shrink-0 rounded-full border border-border px-4 py-2 text-sm font-bold transition hover:bg-muted">{isSuggesting ? 'Hide form' : 'Suggest a cake'}</button></div>{isSuggesting && <form onSubmit={submitSuggestion} className="mt-5 grid gap-4 border-t border-border pt-5 sm:grid-cols-2"><label className="flex flex-col gap-2 text-sm font-semibold">Date<input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} required className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><label className="flex flex-col gap-2 text-sm font-semibold">Your name<input value={suggestionName} onChange={(event) => setSuggestionName(event.target.value)} required className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><label className="flex flex-col gap-2 text-sm font-semibold sm:col-span-2">Reason<textarea value={suggestionReason} onChange={(event) => setSuggestionReason(event.target.value)} placeholder="Birthday, team celebration, or just because..." required rows={3} className="rounded-lg border border-input bg-background px-3 py-2 font-normal" /></label><div className="flex justify-end sm:col-span-2"><button className="rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground">Send suggestion</button></div></form>}</section>}
