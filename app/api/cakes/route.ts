...
export async function POST(request: Request) {
  ...
  if (!cakeDate || !personName || personName.length > 100) {
    return NextResponse.json({ error: 'Please provide a date and a name.' }, { status: 400 })
  }

  const result = await db.execute(sql`
    INSERT INTO cake_events (cake_date, person_name, notes)
    VALUES (${cakeDate}, ${personName}, ${notes || null})
    RETURNING id, cake_date, person_name, notes
  `)
